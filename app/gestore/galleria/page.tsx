...
/**
 * Disabilita la cache statica per questa pagina.
 * Obbliga Next.js a renderizzare la pagina ad ogni richiesta (SSR),
 * fondamentale poiché i dati dipendono dalla sessione utente e dal database in tempo reale.
 */
...
export default async function GestoreGalleriaPage({ searchParams }: PageProps) {
  // --- 1. VERIFICA AUTENTICAZIONE (Chi sei?) ---
  ...
  // Inizializzazione connessione al database SQLite locale
  const dbPath = path.resolve(process.cwd(), 'database.db');
  const db = new Database(dbPath);

  // --- 3. VERIFICA AUTORIZZAZIONE (Hai i permessi?) ---
  // Controlla se il ristorante richiesto appartiene effettivamente al gestore loggato.
  const ownership = db
    .prepare(
  ...
  // Se l'utente tenta di accedere all'ID di un ristorante non suo, lo blocchiamo.
  if (!ownership) {
    db.close();
    redirect('/gestore/ristoranti');
  }
  ...
  // --- 4. RECUPERO DATI (Data Fetching) ---
  // Estrae tutte le immagini associate a questo ristorante, ordinate dalla più recente.
  ...
  // Chiusura connessione per liberare risorse
  db.close();

  return (
    /* Fragment (<> </>) usato per racchiudere elementi multipli senza creare <div> inutili nel DOM */
    <>
      {/* =========================================================================
          SEZIONE 1: SFONDO DECORATIVO GLOBALE
          ========================================================================= */}
      ...
      <div className="flex flex-col py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full relative z-10 min-h-full">
        ...
        <div className="mb-10 border-b-2 border-[#F5CBA7] pb-6">
          <div className="flex items-center gap-4 mb-2">
            ...
            {/* Navigazione Client-Side ottimizzata verso la Dashboard */}
            <Link
        ...
        {/* =========================================================================
            SEZIONE 3: COMPOSIZIONE DEL CLIENT COMPONENT
            ========================================================================= */}
        {/* Inseriamo il mattoncino interattivo passando i dati estratti dal database (props) */}
