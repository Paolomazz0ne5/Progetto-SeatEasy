...
export default function GestoreProfileClient({ initialData }: { initialData: ProfileData }) {
  ...
  const handleDelete = async () => {
    if (confirm("Sei ASSOLUTAMENTE sicuro? Questa azione eliminerà il tuo account, il tuo ristorante e TUTTE le prenotazioni associate irreversibilmente.")) {
      setLoading(true);
      await deleteAccount();
      // the server action will redirect to public home
    }
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-8">
      ...
      {msg && (
        <div className={`p-4 rounded-xl flex items-center gap-3 font-medium animate-in fade-in ${msg.type === 'ok' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>
          {msg.text}
        </div>
      )}
